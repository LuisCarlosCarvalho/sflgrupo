...
export default async function MyPlanPage() {
  ...
  const planDetails: Record<string, { name: string; price: string; features: string[] }> = {
    ...
    PRO: { name: "Profissional", price: "R$ 34,90", features: ["Resolução Full HD", "2 Telas Simultâneas", "Sem Anúncios"] },
    VIP: { name: "VIP Ultra", price: "R$ 54,90", features: ["Resolução 4K + HDR", "4 Telas Simultâneas", "Download para Offline", "Suporte Prioritário"] },
            ...
            <div className="md:col-span-2 glass-panel p-10 rounded-[2.5rem] border-white/5 relative overflow-hidden group">
              ...
              <div className="relative">
                <span className={`inline-block px-4 py-1 rounded-full text-[10px] font-black uppercase tracking-[0.2em] mb-6 border ${
                  isUserActive ? "bg-brand-green/10 text-brand-green border-brand-green/20" : "bg-red-500/10 text-red-500 border-red-500/20"
                }`}>
                  {isUserActive ? "Assinatura Ativa" : "Pagamento Pendente"}
                </span>
